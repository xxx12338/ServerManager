...
echo  ├─────────┬─────┴───┬─────────┬─────────┐
echo  │ 1.开启您的服务器 │ 3.给作者发送反馈 │ 5.禁用关服后重启 │ 7.清空服务端目录 │
echo  ├─────────┼─────────┼─────────┼─────────┤
echo  │ 2.设置服务器信息 │ 4.启用关服后重启 │ 6.魔伊的资源广场 │ 8.退出此管理系统 │
echo  └─────────┴─────────┴─────────┴─────────┘
echo.
set /p doconsole=[INPUT] 请输入您的选择(1/2/3/4/5/6/7/8)：
...
echo java -jar MusicPlayer.jar echo_notitle.mp3>>./tools/player.cmd
...
echo @echo off>./tools/player.cmd
echo title Prismaillya MusicPlayer>>./tools/player.cmd
echo java -jar MusicPlayer.jar colorx3d.mp3>>./tools/player.cmd
echo exit>>./tools/player.cmd
cls
echo.
echo  —— Prismaillya 服务器管理系统 ——————————
...
echo.
choice /C DRE /M "立即下载Java，重新搜索 Java 目录或者退出？(按下D立即下载，R重新搜索，E退出)"
...
echo.
choice /C DRE /M "立即下载组件，重新搜索组件目录或者退出？(按下D立即下载，R重新搜索，E退出)"
