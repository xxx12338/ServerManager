...
@REM 修复窗口大小在 Windows 7 以下显示不正确的问题
...
::┌─────┐
echo  ┌─────┬────┬────────────────────────────┐
echo  │服务器信息│系统提示│  Powered by Prismaillya(https://www.prismaillya.com/)  │
echo  ├─────┴────┴────────────────────────────┘
echo  │当前内存占用：%issueram2%G / %systemrams%G (%ramuse%%%)
echo  │当前登录用户：%USERNAME%
echo  │当前主机名称：%USERDOMAIN%
echo  │服务器外网IP：%localip%
echo  │关服自动重启：%autorestart%
echo  │系统配置信息：%NUMBER_OF_PROCESSORS% 核心处理器，%systemrams%G 内存
echo  ├───────────────┬───────────────────────┐
...
echo.
cd %WORKDIR%\Minecraft
if not exist "plugins" mkdir plugins & echo   ^> plugins 目录不存在，正在创建...
cd ../tools/
